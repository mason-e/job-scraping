...
class JobBase(Parent):
    ...
    SCRIPT = '''INSERT INTO dbo.JobSearchResults ([id], [JobTitle], [Company], [Location], [Summary], [Link], [Source], [Date])
        VALUES'''
    SCRIPT_VALUES = "(@id@, '@Title@', '@Company@', '@Location@', '@Summary@', '@Link@', '@Source@', GETDATE())"
    BLACKLIST = '''DELETE T1
        FROM dbo.JobSearchResults AS T1
        INNER JOIN dbo.Blacklist@BLType@ As T2
        ON T1.[@BLType@] LIKE '%' + T2.[@BLType@] + '%' '''
    DUPLICATES = '''WITH CTE AS (SELECT [Company], [JobTitle], RN = 
        ROW_NUMBER() OVER(PARTITION BY [Company], [JobTitle] ORDER BY [JobTitle]) FROM [JobSearchResults])
    ...
    def get_results_new(self, linkXpath):
        ...
        for result in results:
            ...
            summary = self.get_text_with_validation(result, self.SUMMARY_XPATH)
            link = self.get_link_with_validation(result, linkXpath)
            self.entries.append(JobEntry(title, company, location, summary, link))
    ...
    def get_results(self):
        ...
        self.summaries += self.get_text_results(self.SUMMARY_XPATH)
    ...
    def validate_results(self):
        validation = len(self.titles) == len(self.companies) and \
                     len(self.companies) == len(self.locations) and \
                     len(self.locations) == len(self.summaries)
        ...
        if not validation:
            ...
            print("Summaries found: " + len(self.summaries).__str__())
    ...
    def write_to_db_new(self):
        ...
        for entry in self.entries:
            ...
            script += self.SCRIPT_VALUES.replace("@id@", i.__str__())\
                ...
                .replace("@Summary@", entry.summary.replace("'", "''"))\
    ...
    def write_to_db(self):
        ...
        for title, company, location, summary, link in zip(self.titles, self.companies, self.locations, self.summaries,
                                                           self.links):
            i += 1
